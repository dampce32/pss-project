  ...
  $.fn.reminderInit = function() {
	...
	$(editDialog).dialog({  
	    ...
	    onClose:function(){
			...
			if(rows.length!=0){
				$(reminderDetail).datagrid('loadData',LYS.ClearData);
			}
	    },
	    toolbar:[	
	...
	var setValue = function(){
		...
		$("#reminderDetailIds",editForm).val(reminderDetailIdArray.join(LYS.join));
		$("#reminderItemIds",editForm).val(reminderItemIdArray.join(LYS.join));
		$("#delReminderDetailIds",editForm).val(delReminderDetailIdArray.join(LYS.join));
		return true;
	}
	//保存
	var onSave = function(){
	...
	$(selectDialog).dialog({  
	    ...
	    onClose:function(){
	    	...
	    	$(reminderItemList).datagrid('loadData',LYS.ClearData);
	    }
	});
