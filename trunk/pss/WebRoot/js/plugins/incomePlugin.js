  ...
  $.fn.incomeInit = function() {
	  ...
	  $(viewList).datagrid({
		  ...
		  columns:[[
				...
				{field:'incomeName',title:'收入名称',width:100,align:"center"},
				{field:'incomeDate',title:'收入日期',width:100,align:"center"},
				{field:'bankName',title:'收入银行',width:100,align:"center"},
	...
	var search = function(flag){
		var incomeName = $('#incomeNameSearch',queryContent).val();
		
		var url = "finance/queryIncome.do";
		var content = {incomeName:incomeName,page:pageNumber,rows:pageSize};
	...
	var setValue = function(){
		...
		if(bankId==''){
			$.messager.alert('提示','请选择收入银行','warning');
			return false;
		}
		$('#bankId',editForm).val(bankId);
		var amount = $('#amount',editForm).numberbox('getValue');
		if(amount==''){
			$.messager.alert('提示','请输入收入金额','warning');
			return false;
		}
		if(amount==0){
			$.messager.alert('提示','收入金额不能为0，请重新输入','warning');
			return false;
		}
		//收入日期
		var incomeDate = $('#incomeDate',editForm).val();
		if(incomeDate==''){
			$.messager.alert('提示','请选择收入日期','warning');
			return false;
		}
		//经办人
		...
		$(editForm).form('submit',{
			...
			success: function(data){
				...
				if(result.isSuccess){
					var data = result.data;
					var fn = function(){
						var incomeId = $.trim($('#incomeId',editForm).val());
						if(incomeId==''){//新增
							$('#incomeId',editForm).val(data.incomeId);
							fsBtnStatus();
						}
					}
	...
	var onUpdate = function(){
		...
		initChoose();
		onOpen(selectRow.incomeId);
		$(editDialog).dialog('open');
	 }
	//打开
	var onOpen = function(incomeId){
		var url = 'finance/initIncome.do';
		var content ={incomeId:incomeId};
		asyncCallService(url,content,function(result){
			if(result.isSuccess){
				...
				var data = result.data;
				var incomeData = eval("("+data.incomeData+")");
				$('#incomeId',editDialog).val(incomeData.incomeId);
				$('#incomeName',editDialog).val(incomeData.incomeName);
				$('#incomeDate',editDialog).val(incomeData.incomeDate);
				if(incomeData.status==1){
					shBtnStatus();
				}else{
					fsBtnStatus();
				}
				
				$('#amount',editDialog).numberbox('setValue',incomeData.amount);
				
				$('#bank',editDialog).combobox('setValue',incomeData.bankId);
				$('#employee',editDialog).combobox('setValue',incomeData.employeeId);
				$('#note',editDialog).val(incomeData.note);
			}else{
				$.messager.alert('提示',result.message,'error');
			}
		});
	...
	var onMulDelete = function(){
		...
		for ( var int = 0; int < rows.length; int++) {
			idArray.push(rows[int].incomeId);
		}
		$.messager.confirm("提示","确定要删除选中的记录?",function(t){ 
			if(t){
				var url = 'finance/mulDeleteIncome.do';
	...
	var onUpdateStatus = function(status){
		var incomeId = $('#incomeId',editDialog).val();
		var msg = '';
		if(status==1){
			msg ='审核';
		}else{
		...
		$.messager.confirm("提示","确定要"+msg+"该记录?"+msg+"后系统将进行财务计算!!",function(t){ 
			if(t){
				var url = 'finance/updateStatusIncome.do';
				var content ={incomeId:incomeId,status:status};
				asyncCallService(url,content,function(result){
					if(result.isSuccess){
						var fn = function(){
							if(status==1){
	...
	var onMulUpdateStatus = function(status){
		...
		$.messager.confirm("提示","确定要"+msg+"选中的记录"+msg+"后系统将进行财务计算!!",function(t){ 
			if(t){
				var url = 'finance/mulUpdateStatusIncome.do';
